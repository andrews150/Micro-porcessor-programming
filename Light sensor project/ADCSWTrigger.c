...
void ADC0_InitSWTriggerSeq3_Ch1(void){ volatile unsigned long delay;
  ...
  GPIO_PORTE_DIR_R &= ~0x04;      // 2) make PE2 & PE1 input
...
void ADC1_InitSWTriggerSeq3_Ch1(void){ volatile unsigned long delay;
  SYSCTL_RCGC2_R |= 0x00000010;   // 1) activate clock for Port E
  delay = SYSCTL_RCGC2_R;         //    allow time for clock to stabilize
  ...
  ADC1_SSMUX3_R = (ADC1_SSMUX3_R&0xFFFFFFF0)+2; // 11) channel Ain2 (PE3)
  ADC1_SSCTL3_R = 0x0006;         // 12) no TS0 D0, yes IE0 END0
  ADC1_ACTSS_R |= 0x0008;         // 13) enable sample sequencer 3
}


//Placed this function iside of the interupt handeler
//------------ADC0_InSeq3------------
